...
## Rules and Skills (2026)

- **Rules** (`.cursor/rules/*.mdc`) – Apply by glob or agent selection. Priority: 10 = core, 15 = workflow, 40 = domain, 100 = security. One always-applied rule: `core-engineering.mdc`.
- **Skills** (`.cursor/skills/*/SKILL.md`) – Workflows and domain knowledge. Invoke in Agent chat with `/skill-name` (e.g. `/ralph-workflow`).
- See `.cursor/rules/README.md` for rule format and priority hierarchy.
...
- Use `@rule-name` to apply specific rules from `.cursor/rules/` when needed (rules are `.mdc`; e.g. `@testing`, `@react`)
- Use `/skill-name` in Agent chat to invoke workflow skills from `.cursor/skills/` (e.g. `/ralph-workflow`)
